...
/**
 * A customer sees only their own sites; an admin can list any customer's by
 * passing ?customerId=. Scoped here, not in the route.
 */
...
const getSingleSite = async (siteId: string, user: RequestUser) => {
	...
	if (user.role === Role.CUSTOMER && site.customer.userId !== user.userId) {
		throw new AppError(httpStatus.FORBIDDEN, "This Site Does Not Belong To You");
	}
