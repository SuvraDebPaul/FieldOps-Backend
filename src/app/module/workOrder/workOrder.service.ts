...
/**
 * Approve a request and turn it into a work order.
 *
 * One Serializable transaction: skill match -> daily cap -> create work order
 * -> flip the request to CONVERTED -> history row -> audit row. Either the
 * whole assignment happens or none of it does.
 *
 * Time-overlap is deliberately NOT checked here. The no_technician_double_booking
 * exclusion constraint enforces it inside the database, which closes the race
 * that two dispatchers could otherwise drive through an application-level check.
 */
const approveServiceRequest = async (
	...
	return prisma.$transaction(
		async (tx) => {
			...
			// Skill match: the category names the skill the job needs.
			...
			// Daily job cap, counted over the calendar day of the new booking.
...
const buildWorkOrderFilters = (query: IQuery): WorkOrderWhereInput[] => {
	...
	// Date-window filter on the scheduled start.
...
/**
 * Ownership check, not just a role check: technician A must not be able to read
 * a job assigned to technician B, and a customer must not read another
 * company's job.
 */
const getSingleWorkOrder = async (workOrderId: string, user: RequestUser) => {
	const workOrder = await prisma.workOrder.findFirst({
		where: { id: workOrderId, deletedAt: null },
		include: { ...workOrderInclude, history: { orderBy: { createdAt: "asc" } } },
	});
...
/**
 * The guarded state machine. Three gates, in order:
 *   1. is the transition legal from the current status?
 *   2. is this role allowed to drive it?
 *   3. for a technician, is this actually their job?
 */
	...
	const updated = await prisma.$transaction(async (tx) => {
		const result = await tx.workOrder.update({
			...
			data: {
				...
				// Actual times are stamped by the transition that implies them.
				actualStart:
...
/**
 * Rescheduling re-triggers the exclusion constraint: moving a job onto a slot
 * the technician already occupies raises SQLSTATE 23P01, which the global error
...
const addPartUsage = async (
	...
	// Parts belong to work in progress; once invoiced the totals are frozen.
	if (
